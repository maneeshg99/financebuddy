...
export default function ChartSection({ symbol }: { symbol: string }) {
  ...
  return (
    <section className="rounded-md border border-border bg-panel">
      ...
      >
        <span className="text-muted text-xs uppercase tracking-wider">CHART /// CH-01</span>
      ...
      {open ? (
        <div id="fb-chart-body" className="border-t border-border p-5">
          <PriceChart symbol={symbol} noFrame />
        </div>
      ) : null}
    </section>
  );
